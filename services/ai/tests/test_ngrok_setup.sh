...
if curl -s http://localhost:8080/docs > /dev/null; then
    echo "PASS: Local service is responding"
else
    echo "FAIL: Local service is not responding"
    exit 1
fi
...
if curl -s "$NGROK_URL/docs" -H "ngrok-skip-browser-warning: true" > /dev/null; then
    echo "PASS: Public access via ngrok is working"
else
    echo "FAIL: Public access via ngrok failed"
    exit 1
fi
